 ...
 * @returns lowercase Unicode letter/number key, never empty.
 *
 * Unicode-aware: non-ASCII names (Chinese, accented Latin, …) are preserved
 * so two different CJK member names never collapse to the same fallback key
 * (which previously made the second CJK member collide with the first).
 */
export function sanitizeKey(name: string): string {
  const cleaned = name.trim().toLowerCase().replace(/[^\p{L}\p{N}]+/gu, '-').replace(/^-+|-+$/g, '')
 ...
 * planned and rebuild dependency relationships. When an archive of the same
 * team id already exists (delete → recreate → delete again), the older archive
 * is first moved aside under a timestamped name, because `rename` onto an
 * existing directory fails with EPERM on Windows (ENOTEMPTY on POSIX).
 * @param stateRoot - resolved absolute state root directory.
 * @param teamId - the team id.
 */
export async function archiveTeamDir(stateRoot: string, teamId: string): Promise<void> {
  ...
  if (await pathExists(destination)) {
    let displaced = `${destination}-${Date.now()}`
    let suffix = 0
    while (await pathExists(displaced)) {
      suffix += 1
      displaced = `${destination}-${Date.now()}-${suffix}`
    }
    await rename(destination, displaced)
  }
  await rename(join(stateRoot, teamId), destination)
}

/** Whether a filesystem path exists (access-based). */
