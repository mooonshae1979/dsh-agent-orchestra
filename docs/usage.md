...
> **Anchored Standard 建议**：当 captain 运行在 `anchored-standard` 预设下时，成员通过 scope 继承该预设（先 minimal 双工具锚定 → 晋升后开放全量工具）。此时 `agent_teams_add_member` 的 `mode` 一律用默认 `standard`（不裁剪工具），让锚定→晋升机制完整生效；`mode: 'minimal'` 的 toolFilter 会裁掉晋升后的全量工具，仅用于确实需要单工具极简的一员。
